...
module.exports.stock_update = async(req, res) => {
    ...
    let result = parseFloat(req.body.quantityStocked) - parseFloat(quantityBought);
    // if (quantityBought < req.body.quantityStocked) {

    // }
    try {
        let qtyStocked = Product.findById({ _id: req.params.id }).then((product) => {
            res.send({ product.quantityStocked });
        });
