...
module.exports.stock_update = async(req, res) => {
    const { name, quantityLeft, quantityBought } = req.body;
    try {
        await Product.updateOne({ _id: req.params.id }, { name, quantityLeft: req.params.quantityStocked - quantityBought, dateModified: Date.now() }).then(() => {
