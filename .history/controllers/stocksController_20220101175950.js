...
module.exports.stocks_post = async(req, res) => {
    ...
    try {
        const product = await Product.create({ name, quantityStocked });
...
module.exports.stock_update = async(req, res) => {
    const { name, quantityLeft, dateModified } = req.body;
    try {
        await Product.updateOne({ _id: req.params.id }, { name, quantityLeft: req.params.quantityStocked, dateModified: Date.now() }).then(() => {
